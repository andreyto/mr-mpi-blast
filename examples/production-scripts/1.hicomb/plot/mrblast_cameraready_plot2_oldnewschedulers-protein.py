...
s40k_1000x40_workitemrodering = [133, 60, 30.2, 17.81, 10.37, 6.71]
...
#p400kprotein1024c = [7635.17, 3983.99, 2078.82, 1084.72, 566.11, 294.36]
p400kprotein1024c = [7892.1, 4072.87, 2101.88, 1084.72, 559.79, 294.36, 154.78]
#p400kprotein1024c1 = [1084.72, 566.11, 294.36]
#p400kprotein1024c2 = [7635.17, 3983.99, 2078.82, 1084.72]

p400kprotein1024c1 = [1084.72, 559.79, 294.36, 154.78]
p400kprotein1024c2 = [7892.1, 4072.87, 2101.88, 1084.72]



lw = 1.5

#ax.loglog(x, y1000s80f, 'rd-', linewidth=lw, basex=2, basey=10, label='v4')
...
#ax.loglog(x4, s40k_1000x40_workitemrodering, 'ko-', linewidth=2, basex=2, basey=10, label='v4', markersize=5)
#ax.loglog(x, s40k_1000x40_2nd_scheduler, 'gx-', linewidth=3, basex=2, basey=10, label='v4', markersize=10)

ax.loglog(x6, p400kprotein1024c1, 'rd-', linewidth=lw, basex=2, basey=10, label='v4')
#ax.loglog(x5, p400kprotein1024c2, 'rd-.', linewidth=lw, basex=2, basey=10, label='v4')

ax.set_ylim(1e2, 1e4)
ax.set_xlim(128, 4096)
...
#for i in range(0,len(p400kprotein1024c)):
for i in range(0,4):
    ax.annotate(p400kprotein1024c1[i], xy=(x[i], p400kprotein1024c1[i]),  xycoords='data',
...
#for i in range(0,len(x4)):
    #ax.annotate(s40k_1000x40_workitemrodering[i], xy=(x4[i], s40k_1000x40_workitemrodering[i]),  xycoords='data',
                #xytext=(2, 2), textcoords='offset points', fontsize=datalabelfontsize, color='k')
                ##,arrowprops=dict(arrowstyle="->",connectionstyle="arc3,rad=.2")
                ##)    
...
#ax.legend( (
            ...
            #'40k sequences in 40 blocks (new scheduler)',
...
pylab.savefig('mrblast_plot_prot_2048.png', dpi=(300))                
pylab.show()
