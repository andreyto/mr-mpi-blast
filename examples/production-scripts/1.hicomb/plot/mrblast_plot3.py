...
#ax.loglog(x, y1000s40f, 'rd-.', linewidth=1.5, basex=2, basey=10, label='v5')
#ax.loglog(x, y1000s12f, 'ko-.', linewidth=1.5, basex=2, basey=10, label='v3')
...
for i in range(0,len(x)):
    ax.annotate(y2000s40f[i], xy=(x[i], y2000s40f[i]),  xycoords='data',
                xytext=(-21, -13), textcoords='offset points', fontsize=datalabelfontsize, color='b')
                ##,arrowprops=dict(arrowstyle="->",connectionstyle="arc3,rad=.2")
                ##)
                
#for i in range(0,len(x)):
    #ax.annotate(y1000s40f[i], xy=(x[i], y1000s40f[i]),  xycoords='data',
                #xytext=(-21, -10), textcoords='offset points', fontsize=datalabelfontsize, color='r')
                ###,arrowprops=dict(arrowstyle="->",connectionstyle="arc3,rad=.2")
                ###)
                
#for i in range(0,len(x)):
    #ax.annotate(y1000s12f[i], xy=(x[i], y1000s12f[i]),  xycoords='data',
                #xytext=(-21, -15), textcoords='offset points', fontsize=datalabelfontsize, color='k')
                ###,arrowprops=dict(arrowstyle="->",connectionstyle="arc3,rad=.2")
                ###)
...
pylab.savefig('mrblast_plot3.png', dpi=(300))  
pylab.show()
