...
#path=str(sys.argv[1])
path="./454-25x-reads-2-3-4/"
dirList=os.listdir(path)

#path2=str(sys.argv[2])
path2="./ranger-454-25x-reads-2-3-4-mrblast/"
...
##
## Make each taxid folder in 454_25x_reads_2_3_mrblast
##
for k in taxids.iterkeys():
    outputpath = path2 + "454-25x-2-3-4-" + str(k)
...
for k in taxids.iterkeys():
    input1 = path + str(k) + ".gis.fasta_454_25x_reads_1.fasta"
    input2 = path + str(k) + ".gis.fasta_454_25x_reads_2.fasta"
    outputpath = path2 + "454-25x-2-3-4-" + str(k)
    output = outputpath + "/" + str(k) + ".gis.fasta_454_25x_reads_mrblast.fasta"
    
    if os.path.exists(outputpath + "/" + str(k) + ".gis.fasta_454_25x_reads_mrblast.fasta.idx") and \
       os.path.exists(outputpath + "/" + "mrblast.ini") and \
       os.path.exists(outputpath + "/" + str(k) + ".gis.fasta_454_25x_reads_mrblast.fasta") and \
       os.path.getsize(outputpath + "/" + str(k) + ".gis.fasta_454_25x_reads_mrblast.fasta.idx") > 0 and \
       os.path.getsize(outputpath + "/" + "mrblast.ini") > 0 and \
       os.path.getsize(outputpath + "/" + str(k) + ".gis.fasta_454_25x_reads_mrblast.fasta") > 0:
        ...
        ## make index file
        index_f_name = str(k) + ".gis.fasta_454_25x_reads_mrblast.fasta.idx"
        cmd1 = "python " + make_index_exe + outputpath + "/" + str(k) + ".gis.fasta_454_25x_reads_mrblast.fasta" + " " + outputpath + "/" + index_f_name
        print cmd1
        p = subprocess.Popen(cmd1, shell=True, stdout=subprocess.PIPE, stderr=subprocess.STDOUT)
        stdout_value = p.communicate()[0]
        s = stdout_value.split()
        ...
        #if os.path.exists(outputpath + "/" + str(k) + ".gis.fasta_454_25x_reads_mrblast.fasta.idx") and \
           #os.path.exists(outputpath + "/" + "mrblast.ini") and \
           #os.path.exists(outputpath + "/" + str(k) + ".gis.fasta_454_25x_reads_mrblast.fasta"):
        if os.path.exists(outputpath + "/" + str(k) + ".gis.fasta_454_25x_reads_mrblast.fasta.idx") and \
           os.path.exists(outputpath + "/" + "mrblast.ini") and \
           os.path.exists(outputpath + "/" + str(k) + ".gis.fasta_454_25x_reads_mrblast.fasta") and \
           os.path.getsize(outputpath + "/" + str(k) + ".gis.fasta_454_25x_reads_mrblast.fasta.idx") > 0 and \
           ...
           os.path.getsize(outputpath + "/" + str(k) + ".gis.fasta_454_25x_reads_mrblast.fasta") > 0:
            ...
            f.write("QUERYFILENAME = " + str(k) + ".gis.fasta_454_25x_reads_mrblast.fasta"+"\n")
