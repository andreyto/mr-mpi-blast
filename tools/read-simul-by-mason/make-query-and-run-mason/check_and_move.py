...
for fname in dirList2:
	...
	if (fname.find("illu-100x-4-") > -1):
		move_loc = " /scratch/01471/ssul/40.microbial-illu-100x-4-run-DONE"
	elif (fname.find("454-25x-2-3-4-") > -1):
		move_loc = " /scratch/01471/ssul/41.microbial-454-25x-2-3-4-run-DONE"
	else:
		move_loc = " /scratch/01471/ssul/39.microbial-illu-100x-2-3-run-DONE2"
		
    if os.path.exists(path2 + fname +"/output-workitems.txt"):
        if os.path.exists(path2 + fname +"/SUCCESS") and os.path.exists(path2 + fname +"/output-hits-0-0.txt.bin"):
            print fname
            cmd1 = "mv " + path2 + fname + move_loc
	    print cmd1
