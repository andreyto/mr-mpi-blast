...
if __name__ == '__main__':
    ...
    class BlHits(t.IsDescription):
        ...
        percIdent   = t.FloatCol()      # d: double  # Percentage identity for query (identity count/query length * 100)
        percCover   = t.FloatCol()      # d: double  # Percentage coverage for subject ((qend-qstart)/query length * 100)
    ...
    for i in range(numHitFiles):
        ...
        while True:
            try:
                ...
                BlHits['percIdent']   = s[12]
                BlHits['percCover']   = s[13]
                BlHits.append()
                recordData = hitFile.read(structSize)
            except struct.error:
                break
...
"""
    ...
    for r in table.iterrows():
        print r['qId'], r['eValue'], r['percIdent'], r['percCover'] 
        #print [r[i] for i in range(0,14)]
