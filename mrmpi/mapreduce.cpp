                        ...
                        void *appptr, int addflag)
{
    ...
    while (1) {
        if (me == 0) {
            ...
            if (numItem == vWorkItems.size()-1) n = 0;
            else {
     ...
     */
        ...
        if (me == 0) {
            ...
            int ndone = 0;
            int itask = 0;
            int numNodes = nprocs / 16;
            int numDBPerNode = int(ceil(NUMTOTALDBCHUNKS / numNodes));
                        
            std::map<std::string, unsigned int> mDBAssign;                        
            
            for (int iproc = 1; iproc < nprocs; iproc++) {
                if (itask < nmap) {
                    std::cout << "vWorkItems[itask] = " << vWorkItems[itask] << ", " << iproc << std::endl;
                    vector<string> vTemp = split3(vWorkItems[itask], ',');
                    mDBAssign[vTemp[1]] = iproc;
                    MPI_Send(&itask, 1, MPI_INT, iproc, 0, comm);
                    itask++;
                }
                else {
            ...
            /// DEBUG
            for(std::map<std::string, unsigned int>::iterator im = mDBAssign.begin(); im != mDBAssign.end(); ++im) {
